...
zone = open("/root/dnspriv/zone_files/netmode.ece.ntua.gr_zone", "r")
...
print("Number of Distinct FQDNs in netmode: ", number_of_fqdns)
...
'''
# The DNS zone to compare with
zone = open("ru.zone", "r")
fqdns_ru = set()
for item in zone:
    item = item.rstrip()
    fqdn = item.split("\t")[0].split(" ")[0]
    fqdns_ru.add(fqdn.lower())

# Find the number of unique FQDNs in the zone ru
number_of_fqdns = len(fqdns_ru)
print("Number of Distinct FQDNs in ru zone: ", number_of_fqdns)

# Find false positives comparing to ru zone
matches = 0
for fqdn in fqdns_ru:
    if cuckoo.contains(fqdn):
        matches += 1

print("Matched false positives: ", matches)
'''
