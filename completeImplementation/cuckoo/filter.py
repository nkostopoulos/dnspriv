...
class CuckooFilter(CuckooTemplate):
    ...
    def export(self):
        ...
        for i in range(0, self.capacity):
            if self.buckets[i] != None:
                return_list.append(self.buckets[i].exportItems())
            else:
                return_list.append([])
        return return_list
    ...
    def get_fingerprint(self, item):
        ...
        fingerprint_decimal = hex(int(fingerprint.to01(),2)).split('x')[-1]

        index = self.indices(item, fingerprint)
        indices = list()
        for item in index:
            indices.append(item)

        return (fingerprint_decimal, indices)
